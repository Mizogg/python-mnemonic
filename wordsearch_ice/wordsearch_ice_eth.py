...
def xBal2(ethadd):
    urlblock = "https://ethereum.atomicwallet.io/address/" + ethadd
    respone_block = requests.get(urlblock)
    byte_string = respone_block.content
    source_code = html.fromstring(byte_string)
...
def data_wallet():
    for child in range(0,divs):
        ...
        ethadd = ice.privatekey_to_ETH_address(dec)
        data.append({
...
[purple]  Example BITCOIN Derivation Path[/purple] (m/44'/0'/0'/0/0/ to m/44'/0'/0'/0/999/)
[purple]  Example ETH Derivation Path[/purple] (m/44'/60'/0'/0/0/ to m/44'/60'/0'/0/999/)
[yellow]Enter Your Choice 1-????[/yellow] ✍️ -> ''')
...
while True:
    ...
    if start == 2:
        choice = ' '.join(random.sample(mynumbers, random.randint(1,12)))
        #choice = ''.join(random.sample(mynumbers, random.randint(1,12))) # no space
    if start == 3:
    ...
    if start == 4:
        ...
        choice = "".join(c1+' '+c2+' '+c3+' '+c4+' '+c5+' '+c6+' '+c7+' '+c8+' '+c9+' '+c10+' '+c11+' '+c12+' '+c13+' '+c14+' '+c15+' '+c16+' '+c17+' '+c18+' '+c19+' '+c20+' '+c21+' '+c22+' '+c23+' '+c24)
    data = []
    mnemonic_words = choice
